...
export default function ShoppingCart() {
    ...
    return (
        <div
            className={`bg-white flex flex-col absolute right-3 md:right-9 top-14 w80 py-4 px-4 shadow-[0_5px_15px_0_rgba(0,0,0,.15)] rounded-md transitionopacity duration-500 ${shouldDisplayCart ? "opacity-100" : "opacity-0"
                }`}
        >
            ...
            {cartCount && cartCount > 0 ? (
                <>
                    ...
                    <article className="mt-3 flex flex-col">
                        ...
                        <button
                            onClick={handleClick}
                            className="bg-emerald-50 hover:bg-emerald-500 hover:text-white transition-colors duration500 text-emerald-500 py-3 px-5 rounded-md w-100 disabled:bg-slate-300 disabled:cursor-not-allowed disabled:text-white"
                            disabled={
                                (totalPrice && totalPrice < 30) ||
                                    (cartCount && cartCount > 20) ||
                                    status == "no-items"
